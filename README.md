...
- USB·내장 카메라는 로컬 목록에서 고릅니다. 가능한 경우 `/dev/v4l/by-id` 같은 안정 ID를 저장하므로, 재시작 후 인덱스가 바뀌어도 다른 카메라를 열지 않습니다.
- 같은 물리 장치의 V4L 노드가 여러 개여도 한 항목으로 묶습니다. 확실히 구분되지 않으면 선택을 막고 다시 고르도록 안내합니다.
- 이미 수집 중인 장치의 미리보기는 기존 수집 스레드를 재사용하고, 같은 장치를 다시 열지 않습니다. 탐색·미리보기는 작업 스레드에서 수행합니다.
...
- USB/내장 웹캠의 OS 열거·미리보기·재연결, 같은 이름 실장치 구분
- 사용 중인 카메라 미리보기에서 장치 파일을 두 번 열지 않는지 (코드상 수집 재사용, 실장치로 미확인)
